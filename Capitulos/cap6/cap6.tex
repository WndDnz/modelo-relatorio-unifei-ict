...
Todas são elementos \textbf{opcionais}, e são chamadas quando houver o que listar.

\textbf{Lista sem item algum não é emitida.} Se você chamar \verb|\listoffigures| num documento sem
nenhuma figura, não sai página, nem título, nem entrada de sumário --- em lugar disso, a compilação
registra no log um aviso deste pacote nomeando a lista que foi pulada. O aviso existe porque o
modelo nunca chama lista por conta própria: se a chamada está lá, alguém a escreveu, e sumir em
silêncio deixaria você sem saber por quê.

O aviso é a resposta a essa pergunta, e some sozinho quando a primeira ilustração daquele tipo
entrar no texto. Não é preciso lembrar de descomentar coisa alguma depois: a lista passa a sair
quando passa a ter conteúdo.

Nos arquivos de partida as quatro chamadas continuam comentadas, mas agora por outro motivo. Não é
mais para evitar página em branco --- é para que a primeira compilação de um trabalho recém-copiado
não venha com quatro avisos sobre listas que ninguém ainda pediu. Descomente as que o seu trabalho
vai usar, quando quiser, sem esperar ter a primeira figura.

O mesmo critério vale para os glossários: \verb|\printglossary| sem nenhuma sigla citada no texto
não emite página nem título, e deixa no log o mesmo aviso, nomeando a lista pulada. É a mesma
pergunta feita ao autor nos dois mecanismos, e a resposta é a mesma.

Repare no que o critério mede: não é quantas siglas você \emph{declarou}, é quantas foram
\emph{citadas} no texto. Uma sigla declarada com \verb|\newacronym| e nunca usada com \verb|\gls|
não entra na lista --- e se nenhuma for usada, a lista não sai, por mais siglas que o preâmbulo
declare.
