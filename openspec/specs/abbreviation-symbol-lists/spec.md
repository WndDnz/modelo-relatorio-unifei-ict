# abbreviation-symbol-lists Specification

## Purpose

Garante que a lista de abreviaturas e siglas (ABNT NBR 14724:2024 §4.2.1.7) e a lista de símbolos (§4.2.1.8) declaradas pelo autor cheguem impressas ao documento — o que depende de um passo de compilação próprio, o `makeglossaries`, entre uma passada do LaTeX e a seguinte — e que a lista sem item algum a listar não seja emitida, pelo mesmo critério de elemento opcional vazio que `illustration-types` aplica às listas de ilustração.
