...
necessário, utilizando o comando \verb|\subimport| do pacote \texttt{import}. Assim, para economizar no tempo de compilação durante a edição, você pode comentar temporariamente a importação
...
Este modelo é baseado na classe \texttt{book} do \LaTeX. A ABNT NBR 14724:2024 (4.2.2), porém, não
admite que um trabalho acadêmico seja dividido em \emph{capítulos}: suas divisões chamam-se seções. O
modelo concilia as duas coisas \textbf{rebaixando cada nível um degrau} --- você digita o comando da
classe \texttt{book}, e o documento imprime o nome que a norma exige:

\begin{itemize}
    \item \verb|\chapter| imprime \textbf{Seção} (divisão primária);
    \item \verb|\section| imprime \textbf{Subseção};
    \item \verb|\subsection| imprime \textbf{Subsubseção};
    \item \verb|\subsubsection| imprime \textbf{Parágrafo} (nível mais baixo numerado).
\end{itemize}

Guarde essa correspondência: ela vale para os títulos, para o sumário e para as referências cruzadas.
Ao escrever ``conforme a seção anterior'', lembre-se de que o comando que produziu aquela divisão foi
o \verb|\chapter|. É a única tradução mental que o modelo pede de você.

A seguir, as diretrizes para cada nível. Os prefixos de rótulo sugeridos são mnemônicos do
\textbf{comando}, e não do nome impresso --- por isso \texttt{ssc:} marca aquilo que o documento
chama de Subsubseção:
...
margem esquerda, sem aspas e com a mesma tipografia do corpo do texto, porém em tamanho menor.
